...
public class SpectralClusterer extends AbstractClusterer {
	...
	protected boolean distanceFuncIsSimFunc = false;
	...
	public int clusterInstance(final Instance instance) {
		...
		if(this.distanceFuncIsSimFunc)
		{
			double max_sim = Double.NEGATIVE_INFINITY;
			int c = -1;
			for (int i = 0; i < getData().numInstances(); i++) {
				final double sim = getDistanceFunction().distance(instance,
						getData().instance(i));
				if (sim > max_sim) {
					c = cluster[i];
					max_sim = sim;
	 ...
	 */
	public void setAlphaStar(final double alphaStar)  {
		if ((alphaStar > 0) && (alphaStar < 1))
			this.alphaStar = alphaStar;
		else
			throw new RuntimeException("alpha_star must be between 0 and 1");
	}
	...
	public void setDistanceFunctionIsSimilarityFunction(boolean disFuncIsSimFunc)
	{
		this.distanceFuncIsSimFunc = disFuncIsSimFunc;
	}
